...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  'Draft':       { label: 'DRAFT',      color: 'text-neutral-400',  bg: 'bg-[#1C2030]',        border: 'border-[#252A3A]' },
  'In Review':   { label: 'IN REVIEW',  color: 'text-amber-400',    bg: 'bg-amber-500/10',     border: 'border-amber-500/20' },
  'QA Complete': { label: 'QA COMPLETE',color: 'text-blue-400',     bg: 'bg-blue-500/10',      border: 'border-blue-500/20' },
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={label} className="bg-[#0E1118] rounded-lg p-3 border border-[#1C2030]">
      ...
      content: (
        <div className="space-y-4">
          <div className="flex items-end gap-3">
            <span className="text-4xl font-bold text-emerald-400">${totalSavings.toLocaleString()}</span>
            <span className="text-sm text-neutral-400 mb-1">/ year guaranteed savings</span>
          </div>
          <div className="w-full bg-[#0E1118] rounded-full h-3 border border-[#1C2030] overflow-hidden">
            <div className="bg-emerald-500 h-full rounded-full progress-bar-fill" style={{ width: '94%' }} />
          </div>
          <p className="text-xs text-neutral-400">Achievement rate: <span className="text-emerald-400 font-semibold">94%</span> of guaranteed baseline</p>
        </div>
      ),
      ...
      content: (
        <div className="space-y-3">
          {[
            ...
            <div key={label} className="flex items-center justify-between py-2 border-b border-[#1C2030] last:border-0">
              <span className="text-sm text-neutral-400">{label}</span>
              <span className={cn("text-sm font-bold", highlight ? "text-emerald-400" : "text-white")}>{val}</span>
            </div>
          ))}
        </div>
      ),
        ...
        <div className="space-y-2">
          {risks.slice(0, 3).map((risk) => (
            <div key={risk.id} className={cn(
              ...
              <div>
                <p className="text-xs text-neutral-300 font-medium">{risk.description}</p>
                <p className="text-[10px] text-neutral-500 mt-0.5">{risk.status}</p>
              </div>
            </div>
          ))}
          {risks.length === 0 && <p className="text-sm text-neutral-500">No active risks identified.</p>}
        </div>
      ),
      ...
      content: (
        <div className="space-y-4">
          <div className="flex items-end gap-2">
            <span className="text-4xl font-bold text-emerald-400 animate-stat-pop">{co2Reduced.toLocaleString()}</span>
            <span className="text-sm text-neutral-400 mb-1">tCO₂e / year</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="bg-emerald-500/5 border border-emerald-500/15 rounded-lg p-3">
                ...
                <p className="text-sm font-semibold text-emerald-300 mt-1">{val}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      content: (
        <div className="space-y-2">
          {[
            ...
            <div key={text} className="flex items-center gap-3 py-2 border-b border-[#1C2030] last:border-0">
              {done
                ? <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0" />
                : <CircleDot className="w-4 h-4 text-neutral-500 flex-shrink-0" />
              }
              <span className={cn("text-sm", done ? "text-neutral-400 line-through" : "text-neutral-200")}>{text}</span>
            </div>
          ))}
        </div>
      ),
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-2">
        <p className="text-sm text-neutral-400">Slide-by-slide presentation preview — copy each card to clipboard</p>
        <span className="text-xs text-neutral-500">{slides.length} slides</span>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 stagger-children">
        {slides.map((slide, idx) => (
          <div
            key={slide.title}
            className="card-hover bg-[#12151C] border border-[#1C2030] rounded-xl overflow-hidden flex flex-col"
          >
            <div className="p-4 border-b border-[#1C2030] bg-[#0E1118] flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                {slide.icon}
                <div>
                  <p className="text-[10px] text-neutral-500 uppercase tracking-wider">Slide {idx + 1}</p>
                  <h4 className="text-sm font-bold text-white">{slide.title}</h4>
                </div>
              </div>
              <button
                onClick={() => handleCopy(idx, slide.copyText)}
                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-[#1C2030] border border-[#252A3A] rounded-lg text-xs text-neutral-400 hover:text-white hover:border-[#2D3348] transition-all duration-150"
              >
                {copiedSlide === idx
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-neutral-400">Single-page formatted summary — designed for printing or email</p>
        <div className="flex items-center gap-3">
          <button
            onClick={handleGenerate}
            disabled={generating}
            className="btn-primary inline-flex items-center gap-2 px-4 py-2 bg-[#1C2030] border border-[#252A3A] rounded-lg text-sm text-white hover:bg-[#252A3A] transition-colors duration-150 disabled:opacity-50"
          >
        ...
        <div className="flex-shrink-0 border-b border-[#1C2030] bg-[#12151C] px-8 py-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <select
              ...
              className="bg-[#1C2030] border border-[#252A3A] text-white text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block w-64 p-2.5 transition-colors"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-[#12151C] border border-[#1C2030] rounded-xl p-6">
                <h3 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-4">Report Type</h3>
                <div className="space-y-1.5">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        reportType === type
                          ...
                          : "bg-[#0E1118] text-neutral-400 border-[#1C2030] hover:bg-[#181C25] hover:text-white hover:border-[#252A3A]"
                      )}
                    >
                      {type}
            ...
            <div className="lg:col-span-2">
              {reportType === 'Council Presentation' && selectedProject ? (
                <div className="bg-[#12151C] border border-[#1C2030] rounded-xl overflow-hidden">
                  <div className="p-4 border-b border-[#1C2030] bg-[#0E1118] flex items-center justify-between">
                    <h3 className="text-sm font-medium text-neutral-400">Preview: Council/Board Presentation</h3>
                ...
                <div className="bg-[#12151C] border border-[#1C2030] rounded-xl overflow-hidden">
                  <div className="p-4 border-b border-[#1C2030] bg-[#0E1118] flex items-center justify-between">
                    <h3 className="text-sm font-medium text-neutral-400">Preview: One-Page Executive Summary</h3>
                    <StatusBadge status="Draft" />
                  </div>
                  <div className="p-6 overflow-y-auto max-h-[700px]">
                ...
                <div className="bg-[#12151C] border border-[#1C2030] rounded-xl overflow-hidden h-full min-h-[600px] flex flex-col">
                  <div className="p-4 border-b border-[#1C2030] bg-[#0E1118] flex items-center justify-between">
                    ...
                    <button className="p-1.5 text-neutral-400 hover:text-white hover:bg-[#1C2030] rounded transition-colors duration-150">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-[#1C2030] stagger-rows">
                  {reports.map((report) => (
                    <tr key={report.id} className="hover:bg-[#181C25] transition-colors duration-100">
                      ...
                      <td className="px-6 py-4 text-neutral-300 font-mono text-xs">{report.date}</td>
        ...
        {activeTab === 'qa' && (
          <div className="space-y-8">
            {reports.map((report) => {
              ...
              return (
                <div key={report.id} className="card-hover bg-[#12151C] border border-[#1C2030] rounded-xl overflow-hidden">
                  {/* Header */}
                  <div className="p-6 border-b border-[#1C2030] bg-[#0E1118] flex items-start justify-between">
                    <div>
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-xs font-mono text-neutral-400">{completedCount}/{qaItems.length} items</span>
                      </div>
                      <div className="w-full h-2 bg-[#1C2030] rounded-full overflow-hidden mb-4">
                        <div
                          className={cn(
                          ...
                          <label
                            ...
                            className={cn(
                              ...
                              item.checked
                                ...
                                : "bg-[#0E1118] border-[#1C2030] hover:border-[#252A3A]"
                            )}
                          >
                            <input
                              type="checkbox"
                              ...
                              className="mt-0.5 rounded border-[#252A3A] text-emerald-500 focus:ring-emerald-500 bg-[#1C2030] flex-shrink-0"
                            />
                            <div className="flex-1 min-w-0">
                              <span className={cn(
                                "text-sm block",
                                item.checked ? "text-neutral-400 line-through" : "text-neutral-200"
                              )}>
                                {item.text}
                              </span>
                    ...
                    <div>
                      <button
                        ...
                        className="flex items-center gap-2 text-sm font-semibold text-neutral-300 hover:text-white transition-colors duration-150 mb-3"
                      >
                        <MessageSquare className="w-4 h-4" />
                        Review Comments
                        <span className="text-xs bg-[#1C2030] text-neutral-400 px-1.5 py-0.5 rounded border border-[#252A3A] font-mono">
                          {comments.length}
                        </span>
                          ...
                          {comments.map((comment: any) => (
                            <div
                              ...
                              className={cn(
                                ...
                                comment.resolved
                                  ? "bg-[#0E1118] border-[#1C2030] opacity-70"
                                  : "bg-[#12151C] border-[#252A3A]"
                              )}
                            >
                          ...
                          {!isApproved && (
                            <div className="flex items-center gap-2 pt-1">
                              <input
                                ...
                                className="flex-1 bg-[#0E1118] border border-[#1C2030] text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent placeholder-neutral-600 transition-colors"
                              />
                              <button
                    ...
                    {isApproved && approvedBy && (
                      <div className="flex items-center gap-3 p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-lg">
                        ...
                        <p className="text-sm text-emerald-400">
                          ...
                          <span className="text-neutral-500"> — Report locked</span>
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Footer Actions */}
                  {!isApproved && (
                    <div className="px-6 py-4 border-t border-[#1C2030] bg-[#0E1118] flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className="px-4 py-2 bg-[#1C2030] text-white text-sm font-medium rounded-lg hover:bg-[#252A3A] transition-colors duration-150 border border-[#252A3A]"
                        >
                          Add Comment
                        </button>
                        <button
                          ...
                          className={cn(
                            ...
                            progress === 100
                              ...
                              : "bg-[#1C2030] text-neutral-500 cursor-not-allowed border-[#1C2030]"
                          )}
