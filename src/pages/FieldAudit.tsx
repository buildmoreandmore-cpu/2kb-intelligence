        ...
        <div className="flex-shrink-0 border-b border-[#1C2030] bg-[#12151C] px-8 py-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-neutral-400 mt-1">Capture equipment data, transcribe notes, and flag deficiencies.</p>
            </div>
        ...
        {activeTab === 'assets' ? (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredAssets.map((asset) => (
                <div key={asset.id} className="bg-[#12151C] border border-[#1C2030] rounded-xl overflow-hidden hover:border-emerald-500/50 transition-colors group cursor-pointer">
                  <div className="h-48 relative bg-[#0E1118]">
                    <img 
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h3 className="text-lg font-semibold text-white group-hover:text-emerald-400 transition-colors">{asset.type}</h3>
                        <p className="text-sm text-neutral-400">{buildings.find(b => b.id === asset.buildingId)?.name}</p>
                      </div>
                      <span className={cn(
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
                      <div>
                        <span className="block text-neutral-500 text-xs font-medium uppercase tracking-wider mb-1">Make/Model</span>
                        <span className="text-neutral-300 font-medium">{asset.manufacturer} {asset.model}</span>
                      </div>
                      <div>
                        <span className="block text-neutral-500 text-xs font-medium uppercase tracking-wider mb-1">Year</span>
                        <span className="text-neutral-300 font-medium">{asset.year}</span>
                      </div>
                    </div>
              ...
              {filteredAssets.length === 0 && (
                <div className="col-span-full p-8 text-center text-neutral-500">No assets found.</div>
              )}
            </div>
          </div>
        ) : (
          <div className="max-w-3xl mx-auto">
            <div className="bg-[#12151C] border border-[#1C2030] rounded-xl overflow-hidden">
              <div className="p-6 border-b border-[#1C2030] bg-[#0E1118]">
                <h3 className="text-lg font-medium text-white mb-2">AI Extraction Queue</h3>
                <p className="text-sm text-neutral-400">Upload nameplate photos or record voice notes. Claude will extract structured data and flag deficiencies automatically.</p>
              </div>
              
              <div className="p-8">
                <div className="border-2 border-dashed border-[#252A3A] rounded-xl p-12 flex flex-col items-center justify-center text-center hover:bg-[#181C25] hover:border-emerald-500/50 transition-colors cursor-pointer group">
                  <div className="w-16 h-16 bg-[#1C2030] text-emerald-500 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    <Camera className="w-8 h-8" />
                  </div>
                  <h4 className="text-lg font-medium text-white mb-1">Upload Equipment Photos</h4>
                  <p className="text-sm text-neutral-400 max-w-sm">Drag and drop nameplate photos, wide shots, or inspection documents here.</p>
                </div>

                <div className="mt-8 flex items-center gap-4">
                  <div className="flex-1 h-px bg-[#1C2030]"></div>
                  <span className="text-sm font-medium text-neutral-500 uppercase tracking-wider">OR</span>
                  <div className="flex-1 h-px bg-[#1C2030]"></div>
                </div>

                <div className="mt-8 text-center">
                  <button className="inline-flex items-center gap-2 px-6 py-3 bg-[#1C2030] border border-[#252A3A] rounded-full text-white font-medium hover:border-emerald-500 hover:text-emerald-400 transition-colors shadow-sm">
                    <Mic className="w-5 h-5" />
                    Record Voice Audit Note
                  </button>
                  <p className="text-xs text-neutral-500 mt-3">Whisper will transcribe and Claude will structure the data.</p>
                </div>
              </div>
            </div>
          </div>
