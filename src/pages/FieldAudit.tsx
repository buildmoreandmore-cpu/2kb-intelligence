      ...
      {!projectId && (
        <div className="flex-shrink-0 border-b border-[#EAEDF3] bg-[#FFFFFF] px-8 py-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Field Audit & Asset Intelligence</h1>
              <p className="text-sm text-gray-500 mt-1">Capture equipment data, transcribe notes, and flag deficiencies.</p>
            </div>
        ...
        {activeTab === 'assets' ? (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="relative w-96">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-[#FFFFFF] border border-[#EAEDF3] rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent shadow-sm"
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredAssets.map((asset) => (
                <div key={asset.id} className="bg-[#FFFFFF] border border-[#EAEDF3] rounded-xl overflow-hidden hover:border-emerald-500/50 transition-colors group cursor-pointer">
                  <div className="h-48 relative bg-[#F8FAFB]">
                    <img 
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900 group-hover:text-emerald-600 transition-colors">{asset.type}</h3>
                        <p className="text-sm text-gray-500">{buildings.find(b => b.id === asset.buildingId)?.name}</p>
                      </div>
                      <span className={cn(
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
                      <div>
                        <span className="block text-gray-500 text-xs font-medium uppercase tracking-wider mb-1">Make/Model</span>
                        <span className="text-gray-600 font-medium">{asset.manufacturer} {asset.model}</span>
                      </div>
                      <div>
                        <span className="block text-gray-500 text-xs font-medium uppercase tracking-wider mb-1">Year</span>
                        <span className="text-gray-600 font-medium">{asset.year}</span>
                      </div>
                    </div>

                    {asset.flags.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-[#EAEDF3]">
              ...
              {filteredAssets.length === 0 && (
                <div className="col-span-full p-8 text-center text-gray-500">No assets found.</div>
              )}
            </div>
          </div>
        ) : (
          <div className="max-w-3xl mx-auto">
            <div className="bg-[#FFFFFF] border border-[#EAEDF3] rounded-xl overflow-hidden">
              <div className="p-6 border-b border-[#EAEDF3] bg-[#F8FAFB]">
                <h3 className="text-lg font-medium text-gray-900 mb-2">AI Extraction Queue</h3>
                <p className="text-sm text-gray-500">Upload nameplate photos or record voice notes. Claude will extract structured data and flag deficiencies automatically.</p>
              </div>
              ...
              <div className="p-8">
                <div className="border-2 border-dashed border-[#D4D8E2] rounded-xl p-12 flex flex-col items-center justify-center text-center hover:bg-[#F0F2F6] hover:border-emerald-500/50 transition-colors cursor-pointer group">
                  ...
                  <h4 className="text-lg font-medium text-gray-900 mb-1">Upload Equipment Photos</h4>
                  <p className="text-sm text-gray-500 max-w-sm">Drag and drop nameplate photos, wide shots, or inspection documents here.</p>
                </div>
                ...
                <div className="mt-8 text-center">
                  <button className="inline-flex items-center gap-2 px-6 py-3 bg-[#EAEDF3] border border-[#D4D8E2] rounded-full text-gray-600 font-medium hover:border-emerald-500 hover:text-emerald-600 transition-colors shadow-sm">
                    <Mic className="w-5 h-5" />
                    Record Voice Audit Note
                  </button>
                  <p className="text-xs text-gray-500 mt-3">Whisper will transcribe and Claude will structure the data.</p>
                </div>
              </div>
            </div>
          </div>
