        ...
        <div className="flex-shrink-0 border-b border-[#1C2030] bg-[#12151C] px-8 py-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-neutral-400 mt-1">Track installation progress, inspections, and scope deviations.</p>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-7xl mx-auto w-full space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#12151C] border border-[#1C2030] rounded-xl p-6">
            <h3 className="text-sm font-medium text-neutral-400 uppercase tracking-wider mb-2">Total ECMs</h3>
          ...
          <div className="bg-[#12151C] border border-[#1C2030] rounded-xl p-6">
            <h3 className="text-sm font-medium text-neutral-400 uppercase tracking-wider mb-2">Open Findings</h3>
            <div className="flex items-end gap-3">
          ...
          <div className="bg-[#12151C] border border-[#1C2030] rounded-xl p-6">
            <h3 className="text-sm font-medium text-neutral-400 uppercase tracking-wider mb-2">Scope Deviations</h3>
            <div className="flex items-end gap-3">
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-[#1C2030]">
                  {projectEcms.map((ecm, idx) => {
                    ...
                    return (
                      <tr key={ecm.id} className="hover:bg-[#181C25] transition-colors">
                        ...
                        <td className="px-6 py-4 text-neutral-300">{ecm.category}</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-32 h-2 bg-[#1C2030] rounded-full overflow-hidden">
                              <div 
                  ...
                  {projectEcms.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-6 py-8 text-center text-neutral-500">No ECMs found for this project.</td>
                    </tr>
                  )}
                </tbody>
              </table>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-[#1C2030]">
                  {projectFindings.map((finding) => (
                    <tr key={finding.id} className="hover:bg-[#181C25] transition-colors">
                      <td className="px-6 py-4 text-neutral-300 font-mono">{finding.date}</td>
                      <td className="px-6 py-4 text-neutral-300">{finding.ecm}</td>
                      <td className="px-6 py-4 text-neutral-300">{finding.type}</td>
                      <td className="px-6 py-4">
                        <span className={cn(
                          "px-2.5 py-1 rounded text-xs font-medium border",
                  ...
                  {projectFindings.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-6 py-8 text-center text-neutral-500">No inspection findings logged for this project.</td>
                    </tr>
                  )}
                </tbody>
              </table>
