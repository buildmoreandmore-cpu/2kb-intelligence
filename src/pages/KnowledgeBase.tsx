    ...
    <div className="flex flex-col h-full">
      <div className="flex-shrink-0 border-b border-[#EAEDF3] bg-[#FFFFFF] px-8 py-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Knowledge Capture & Institutional Memory</h1>
            <p className="text-sm text-gray-500 mt-1">Store historical data, benchmark costs/savings, prevent repeating mistakes.</p>
          </div>
        </div>
        ...
        {activeTab === 'benchmarks' && (
          <div className="bg-[#FFFFFF] border border-[#EAEDF3] rounded-xl overflow-hidden">
            <div className="p-6 border-b border-[#EAEDF3]">
              <h3 className="text-sm font-semibold text-gray-900">Historical Cost & Savings Benchmarks</h3>
            </div>
                ...
                <tbody className="divide-y divide-[#EAEDF3]">
                  {benchmarks.filter(b => b.category.toLowerCase().includes(searchQuery.toLowerCase())).map((benchmark) => (
                    <tr key={benchmark.id} className="hover:bg-[#F0F2F6] transition-colors">
                      <td className="px-6 py-4 font-medium text-gray-900">{benchmark.category}</td>
                      <td className="px-6 py-4 text-gray-600">{benchmark.buildingType}</td>
                      <td className="px-6 py-4 text-right text-gray-600 font-mono">${benchmark.unitCostMid.toLocaleString()}</td>
                      <td className="px-6 py-4 text-gray-600">{benchmark.unit}</td>
                  ...
                  {benchmarks.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-6 py-8 text-center text-gray-500">No benchmarks found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        {activeTab === 'lessons' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {lessonsLearned.filter(l => l.title.toLowerCase().includes(searchQuery.toLowerCase())).map((lesson) => (
              <div key={lesson.id} className="bg-[#FFFFFF] border border-[#EAEDF3] rounded-xl p-6 flex flex-col hover:border-emerald-500/50 transition-colors group">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 group-hover:text-emerald-600 transition-colors">{lesson.title}</h3>
                    <p className="text-sm text-gray-500 mt-1">Project: {useStore.getState().projects.find(p => p.id === lesson.projectId)?.name}</p>
                  </div>
                  <span className="px-2.5 py-1 rounded bg-[#EAEDF3] text-xs font-medium text-gray-600 border border-[#D4D8E2]">
                    {lesson.category}
                  </span>
                </div>
                <div className="space-y-4 flex-1">
                  ...
                  <div>
                    <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Recommendation</h4>
        ...
        {activeTab === 'templates' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={template.id} className="bg-[#FFFFFF] border border-[#EAEDF3] rounded-xl p-6 flex flex-col hover:border-emerald-500/50 transition-colors group">
                <div className="flex-1">
                  <div className="w-10 h-10 bg-[#EAEDF3] rounded-lg flex items-center justify-center mb-4 group-hover:bg-emerald-500/20 transition-colors">
                    <FileText className="w-5 h-5 text-gray-500 group-hover:text-emerald-500 transition-colors" />
                  </div>
                ...
                <div className="mt-6 pt-4 border-t border-[#EAEDF3] flex items-center justify-between">
                  <span className="text-xs text-gray-500 font-mono">Updated: {template.updated}</span>
