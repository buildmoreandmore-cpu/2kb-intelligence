    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex-shrink-0 border-b border-[#1C2030] bg-[#12151C] px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-neutral-400 mt-1">Build ECM bundles, model cash flows, and analyze guarantee risk.</p>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-7xl mx-auto w-full space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-[#12151C] border border-[#1C2030] rounded-xl overflow-hidden">
            <div className="p-6 border-b border-[#1C2030] flex items-center justify-between">
              ...
              <div className="flex items-center gap-4 text-sm">
                <span className="text-neutral-400">Total Cost: <span className="text-white font-mono">${totalCost.toLocaleString()}</span></span>
                <span className="text-neutral-400">Total Savings: <span className="text-emerald-500 font-mono">${totalSavings.toLocaleString()}/yr</span></span>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-[#1C2030]">
                  {projectEcms.map((ecm) => (
                    <tr key={ecm.id} className="hover:bg-[#181C25] transition-colors">
                      <td className="px-6 py-4 font-medium text-white">
                        <div className="flex flex-col">
                          <span>{ecm.number}</span>
                          <span className="text-xs text-neutral-500 font-normal">{ecm.description}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right text-neutral-300 font-mono">${ecm.cost.toLocaleString()}</td>
                      <td className="px-6 py-4 text-right text-emerald-500 font-mono">${ecm.savings.toLocaleString()}</td>
                      <td className="px-6 py-4 text-right text-neutral-300 font-mono">{(ecm.cost / ecm.savings).toFixed(1)} yrs</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-[#12151C] border border-[#1C2030] rounded-xl p-6 flex flex-col">
            <h3 className="text-sm font-semibold text-white mb-6">Financial Assumptions</h3>
            <div className="space-y-6 flex-1">
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-sm font-medium text-neutral-400">Contract Term (Years)</label>
                ...
                <input 
                  ...
                  className="w-full h-2 bg-[#1C2030] rounded-lg appearance-none cursor-pointer accent-emerald-500"
                />
              </div>
              
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-sm font-medium text-neutral-400">Interest Rate (%)</label>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-sm font-medium text-neutral-400">Electric Escalation (%)</label>
            ...
            <div className="mt-6 pt-6 border-t border-[#1C2030] grid grid-cols-2 gap-4">
              <div>
                <span className="block text-xs text-neutral-500 uppercase tracking-wider mb-1">NPV (5%)</span>
                <span className={`text-lg font-bold font-mono ${npv >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                  ${Math.round(npv).toLocaleString()}
                </span>
              </div>
              <div>
                <span className="block text-xs text-neutral-500 uppercase tracking-wider mb-1">DSCR</span>
                <span className={`text-lg font-bold font-mono ${parseFloat(dscr.toString()) >= 1.0 ? 'text-emerald-500' : 'text-red-500'}`}>
                  {dscr}x
                </span>
              </div>
            </div>
            ...
            {cashFlows.map((cf) => {
              ...
              return (
                <div key={cf.year} className="flex-1 flex flex-col items-center gap-2 group relative h-full justify-end">
                  ...
                  <span className="text-[10px] text-neutral-500 font-mono">{cf.year}</span>
                </div>
              );
            })}
          </div>
