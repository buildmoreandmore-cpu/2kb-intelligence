            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white border border-[#EAEDF3] rounded-xl p-5">
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Total Obligations</p>
                ...
                <div className="bg-white border border-[#EAEDF3] rounded-xl p-5">
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Coming Due</p>
              ...
              <div className="bg-white border border-[#EAEDF3] rounded-xl p-6">
                <h3 className="text-sm font-semibold text-gray-900 mb-4">Compliance Summary</h3>
                <div className="flex items-center gap-8">
                  <div className="relative w-32 h-32 flex-shrink-0">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-xl font-bold text-gray-900">{total > 0 ? Math.round((completed / total) * 100) : 0}%</span>
                    </div>
                  </div>
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center gap-2"><span className="w-3 h-3 bg-emerald-500 rounded-full" /><span className="text-gray-600">{completed} Completed on time</span></div>
                    <div className="flex items-center gap-2"><span className="w-3 h-3 bg-amber-400 rounded-full" /><span className="text-gray-600">{comingDue} Coming due</span></div>
                    <div className="flex items-center gap-2"><span className="w-3 h-3 bg-red-500 rounded-full" /><span className="text-gray-600">{overdue} Overdue</span></div>
                    <div className="flex items-center gap-2"><span className="w-3 h-3 bg-gray-200 rounded-full" /><span className="text-gray-600">{total - completed - comingDue - overdue} Not yet due</span></div>
                  </div>
                </div>
              </div>
              ...
              {upcoming90.length > 0 && (
                <div className="bg-white border border-[#EAEDF3] rounded-xl overflow-hidden">
                  <div className="p-6 border-b border-[#EAEDF3]">
                    <h3 className="text-sm font-semibold text-gray-900">Upcoming & Overdue Obligations</h3>
                  </div>
                      ...
                      <div key={o.id} className="px-6 py-4">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            ...
                            <p className="text-xs text-gray-400 mt-1 italic">Ref: {o.contractRef}</p>
              ...
              <div className="bg-white border border-[#EAEDF3] rounded-xl overflow-hidden">
                <div className="p-6 border-b border-[#EAEDF3] flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-gray-900">All Obligations</h3>
                    ...
                    {categories.map(c => (
                      <button
                        ...
                        className={cn('px-3 py-1.5 text-xs font-medium rounded-lg transition-colors',
                          catFilter === c ? 'bg-emerald-50 text-emerald-700' : 'text-gray-500 hover:bg-gray-100'
                        )}
                      >
                        {c}
                      </button>
                    ...
                    <tbody className="divide-y divide-[#EAEDF3]">
                      {filtered.map(o => (
                        <tr key={o.id} className="hover:bg-[#F0F2F6] transition-colors">
                          <td className="px-6 py-4">
                            <p className="font-medium text-gray-900 max-w-sm">{o.description}</p>
                            {o.evidence && <p className="text-[10px] text-gray-400 mt-0.5">{o.evidence}</p>}
                          </td>
                          ...
                          <td className="px-6 py-4 text-gray-600">{o.responsibleParty}</td>
                          <td className="px-6 py-4 text-gray-600 font-mono text-xs">{o.dueDate || 'Ongoing'}</td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 text-xs text-gray-400">{o.contractRef}</td>
                        </tr>
                      ))}
                      {filtered.length === 0 && (
                        <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">No obligations found.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
